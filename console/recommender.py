...
def cofiCostFunc(myparams, myY, myR, mynu, mynm, mynf, mylambda=0.):
    ...
    # First dot theta and X together such that you get a matrix the same shape
    # as Y
...
# Let's check my gradient computation real quick:#Let's c
def checkGradient(myparams, myY, myR, mynu, mynm, mynf, mylambda=0.):
    ...
    for i in xrange(10):
        ...
        epsvec[idx] = myeps
        loss1 = cofiCostFunc(myparams - epsvec, myY, myR,
                             mynu, mynm, mynf, mylambda)
        loss2 = cofiCostFunc(myparams + epsvec, myY, myR,
                             mynu, mynm, mynf, mylambda)
...
print 'Average rating for book 1 (The Hunger Games): %0.2f' % \
...
# Read in the movie params matrices# Read i
datafile = 'data/booksParams.mat'
mat = scipy.io.loadmat(datafile)
X = mat['X']
Theta = mat['Theta']
...
# "...run your cost function. You should expect to see an output of 22.22."# "...ru
...
for i in xrange(len(my_ratings)):
    if my_ratings[i] > 0:
        print 'Rated %d for book %s.' % (my_ratings[i], movies[i])
